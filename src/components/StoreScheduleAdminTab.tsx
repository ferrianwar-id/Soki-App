  ...
  useEffect(() => {
    if (siteSettings?.storeSchedule && !isSaving && !savingMode) {
      setFormState({
        ...
        weeklySchedule: siteSettings.storeSchedule.weeklySchedule && siteSettings.storeSchedule.weeklySchedule.length > 0
          ? siteSettings.storeSchedule.weeklySchedule
          : defaultWeeklySchedule
      });
    }
  }, [siteSettings?.storeSchedule]);
    ...
    <div className="space-y-6 max-w-5xl">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200/80 space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              ...
              <div className="flex items-center justify-between">
                ...
                {formState.statusMode === 'auto' && (
                  <span className="text-[10px] bg-orange-100 text-orange-700 font-bold px-1.5 py-0.5 rounded-md">
                    {savingMode === 'auto' ? 'Menyimpan...' : '✓ Database'}
                  </span>
                )}
              </div>
          ...
          <div
            onClick={() => handleSelectMode('force_open')}
            className={`p-4 rounded-2xl border-2 transition cursor-pointer flex flex-col justify-between ${
              ...
              <div className="flex items-center justify-between">
                ...
                {formState.statusMode === 'force_open' && (
                  <span className="text-[10px] bg-emerald-100 text-emerald-700 font-bold px-1.5 py-0.5 rounded-md">
                    {savingMode === 'force_open' ? 'Menyimpan...' : '✓ Buka di Database'}
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-500 mt-1 leading-snug">
          ...
          <div
            onClick={() => handleSelectMode('force_closed')}
            className={`p-4 rounded-2xl border-2 transition cursor-pointer flex flex-col justify-between ${
          ...
          >
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                {formState.statusMode === 'force_closed' && (
                  <span className="text-[10px] bg-rose-100 text-rose-700 font-bold px-1.5 py-0.5 rounded-md">
                    {savingMode === 'force_closed' ? 'Menyimpan...' : '✓ Tutup di Database'}
                  </span>
                )}
              </div>
              <p className="text-[11px] text-slate-500 mt-1 leading-snug">
