...
export default function MaintenanceAdminTab({
  ...
  // Synchronize formState if siteSettings updates from background polling or other browsers
  useEffect(() => {
    if (siteSettings?.maintenance && !isSaving) {
      setFormState(siteSettings.maintenance);
    }
  ...
  const handleSave = async (updatedConfig?: MaintenanceConfig) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        onUpdateSettings(data.siteSettings || updatedSettings);
        showStatus(
          configToSave.enabled 
            ? "Mode Maintenance BERHASIL DIAKTIFKAN & tersimpan di database! Semua browser pengunjung akan serentak melihat layar pemeliharaan." 
            : "Mode Maintenance BERHASIL DINONAKTIFKAN & tersimpan di database! Semua browser pengunjung akan serentak kembali melihat toko."
        );
      } else {
