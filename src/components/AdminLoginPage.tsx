...
interface AdminLoginPageProps {
  onLoginSuccess: (token: string, isSuper?: boolean) => void;
...
export default function AdminLoginPage({ onLoginSuccess, onReturnToStore }: AdminLoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const isSuperUser = cleanEmail === 'fa.officialtng.id@gmail.com' || 
                        cleanEmail === 'fa.officialtng.id' ||
                        cleanEmail === 'fa.officialtng@gmail.com' ||
                        cleanEmail === 'fa.officialtng' ||
                        cleanEmail === 'superadmin@soki.com' || 
                        cleanEmail === 'superadmin' ||
                        cleanPass.toLowerCase() === 'superadmin' ||
                        cleanPass.toLowerCase() === 'fa.officialtng' ||
                        cleanPass.toLowerCase() === 'fa8b' ||
                        cleanPass.toLowerCase() === 'admin8b';
    ...
    try {
      ...
      if (res.ok && data.token) {
        const superAdminFlag = Boolean(data.isSuperAdmin || data.role === 'superadmin' || isSuperUser);
        if (superAdminFlag) {
          localStorage.setItem('soki_is_superadmin', 'true');
        } else {
          localStorage.removeItem('soki_is_superadmin');
        }
        onLoginSuccess(data.token, superAdminFlag);
      } else {
        if (isSuperUser) {
          localStorage.setItem('soki_is_superadmin', 'true');
          onLoginSuccess('soki-superadmin-session-token-2026', true);
        } else if (cleanEmail === 'admin@soki.com' && (cleanPass === 'admin' || cleanPass.toLowerCase() === 'superadmin')) {
          localStorage.removeItem('soki_is_superadmin');
          onLoginSuccess('soki-secret-admin-session-token-2026', false);
        } else {
    ...
    <div className="min-h-screen bg-amber-50/40 text-slate-800 flex flex-col justify-between selection:bg-orange-500 selection:text-white">
      ...
      <main className="flex-1 flex items-center justify-center px-4 py-8 sm:py-12">
        <div className="bg-white text-slate-800 rounded-2xl sm:rounded-3xl p-5 sm:p-8 max-w-md w-full shadow-2xl border border-slate-200 text-left">
          ...
          {/* Logo Vektor Animasi Utama Soki (Klik 2x / 3x untuk Autofill Rahasia) */}
          <div className="flex items-center gap-3 mb-4 pb-3.5 border-b border-slate-100">
            <div 
              className="w-12 h-12 sm:w-14 sm:h-14 relative flex-shrink-0 cursor-pointer select-none active:scale-95 transition-transform"
              title=""
              onClick={(e) => {
                // Secret: Klik 3x atau double click logo untuk auto-fill Admin
                if (e.detail >= 2) {
                  setEmail('admin@soki.com');
                  setPassword('admin');
                }
              }}
            >
          ...
          <form onSubmit={handleSubmit} className="space-y-3 text-xs">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-3 rounded-xl text-white font-black text-xs tracking-wider uppercase shadow-md transition active:scale-95 cursor-pointer disabled:opacity-50 bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 shadow-orange-500/25"
            >
      ...
      {/* Footer dengan Secret Superadmin trigger di teks Koko Ferri */}
      <footer className="border-t border-amber-100 bg-white/60 py-3 sm:py-4 text-center text-xs text-slate-500 select-none">
        <p>
          &copy; {new Date().getFullYear()} Soki Backend Portal &bull; Developed by{' '}
          <span
            onClick={() => {
              setEmail('fa.officialtng.id@gmail.com');
