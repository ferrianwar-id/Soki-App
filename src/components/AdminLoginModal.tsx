  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data.token) {
        if (isSuperUser || data.isSuperAdmin || data.role === 'superadmin') {
        ...
        if (isSuperUser) {
          localStorage.setItem('soki_is_superadmin', 'true');
          onLoginSuccess('soki-superadmin-session-token-2026');
          onClose();
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl border border-slate-200 relative text-left">
        ...
        {/* LOGO RESMI SOKI ICON + BADGE ADMIN (Secret Double Click: Autofill) */}
        <div className="flex items-center gap-3.5 mb-5">
          <div 
            className="w-14 h-14 relative flex-shrink-0 cursor-pointer select-none active:scale-95 transition-transform"
            title=""
            onClick={(e) => {
              // Secret: Klik ganda / 3x logo untuk autofill admin
              if (e.detail >= 2) {
          ...
          <div>
            ...
            <span 
              ...
              className="text-[9px] font-extrabold tracking-wider bg-orange-100 text-orange-700 px-2 py-0.5 rounded-full uppercase mt-1 inline-block cursor-pointer select-none"
              title=""
            >
